    ...
    for epoch in range(epochs):
        ...
        for imgs, bboxes, num_b, num_H, num_W, masks in dataloader:
            ...
            if (step <= 10000 and step % 1000 == 0) or step % 5000 == 0 or step == 1:
                torch.save(model.state_dict(), './models/Mask_Rcnn_cascade_50_%d_1.pth' % step)
            if step >= 90010:
                flag = True
                break
        if flag:
            break
    torch.save(model.state_dict(), './models/Mask_Rcnn_cascade_50_final_1.pth')
...
if __name__ == "__main__":
    ...
    config = Config(True, Mean, files, num_cls=80, lr=0.00125, weight_decay=0.0001, batch_size_per_GPU=1,
...
#  Average Precision  (AP) @[ IoU=0.50:0.95 | area= small | maxDets=100 ] = 0.50
