...
from tool.torch_PTC_test import ProposalTargetCreator
...
from tool.read_Data_coco import Read_Data
...
class Faster_Rcnn(nn.Module):
    ...
    def get_loss(self, x, bboxes, num_b, H, W):
        ...
        roi, loc, label = self.fast_train_data(rpn_loc.data, F.softmax(rpn_logits.data, dim=-1)[:, 1],
                                               tanchors,
                                               img_size, bboxes, map_HW)

        # x = self.roialign(P, roi, roi_inds)
    ...
    for epoch in range(epochs):
        ...
        for imgs, bboxes, num_b, num_H, num_W in dataloader:
            ...
            if (step <= 10000 and step % 1000 == 0) or step % 5000 == 0 or step == 1:
                torch.save(model.state_dict(), './models/FPN_coco_50_%d_1.pth' % step)
        ...
        if flag:
            break
    torch.save(model.state_dict(), './models/FPN_coco_50_final_1.pth')
...
def train_dist(model, config, step, x, pre_model_file, model_file=None):
    ...
    dataloader = DataLoader(dataset, batch_size=config.batch_size_per_GPU, sampler=train_sampler,
                            collate_fn=func, drop_last=True, pin_memory=True, num_workers=8)
    scheduler = WarmupMultiStepLR(opt, [60000 * x, 80000 * x], warmup_factor=1 / 3, warmup_iters=500)
    epochs = 10000
    flag = False
    print('start:  step=', step)
        ...
        for imgs, bboxes, num_b, num_H, num_W in dataloader:
            ...
            if ((step <= 10000 and step % 1000 == 0) or step % 5000 == 0 or step == 1) and local_rank == 0:
                torch.save(model.module.state_dict(),
                           './models/FPN_coco_50_%dx_%d_1_%d.pth' % (x, step, local_rank))
            if step >= 90010 * x:
                flag = True
                break
        if flag:
    ...
    if local_rank == 0:
        torch.save(model.module.state_dict(),
                   './models/FPN_coco_50_%dx_final_1_%d.pth' % (x, local_rank))
