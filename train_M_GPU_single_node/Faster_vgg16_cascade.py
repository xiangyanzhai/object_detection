...
from tool.torch_ATC_test import AnchorTargetCreator
...
def train(model, config, step, x, pre_model_file, model_file=None):
    ...
    dataloader = DataLoader(dataset, batch_size=config.batch_size_per_GPU, collate_fn=func,
                            shuffle=True, drop_last=True, pin_memory=True, num_workers=6)
    ...
    for epoch in range(epochs):
        for imgs, bboxes, num_b, num_H, num_W in dataloader:
            ...
            if (step <= 10000 and step % 1000 == 0) or step % 5000 == 0 or step == 1:
                torch.save(model.state_dict(), './models/vgg16_cascade_%d_1.pth' % step)
            if step >= 90010 * x:
                flag = True
                break
        if flag:
            break
    torch.save(model.state_dict(), './models/vgg16_cascade_final_1.pth')
