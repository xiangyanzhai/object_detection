...
from tool.torch_PTC_test import ProposalTargetCreator as ProposalTargetCreator_box
...
class Mask_Rcnn(nn.Module):
    ...
    def get_loss(self, x, bboxes, num_b, H, W, masks):
        ...
        with torch.no_grad():
            ...
            roi, loc, label = self.fast_train_data_box(roi, bboxes, self.PTC_1, self.loc_std1)
        ...
        with torch.no_grad():
            ...
            roi = self.filter_bboxes(roi, img_size, self.config.roi_min_size[0])
            roi, loc, label = self.fast_train_data_box(roi, bboxes, self.PTC_2, self.loc_std2)
        ...
        with torch.no_grad():
            ...
            roi = self.loc2bbox(fast_loc, roi)
            roi = self.filter_bboxes(roi, img_size, self.config.roi_min_size[0])
            roi, loc, label, inter, target = self.fast_train_data(roi, bboxes, masks, img_size,
                                                                  self.loc_std3)
        ...
        self.d = box_loss
        self.f = mask_loss
        return rpn_cls_loss + rpn_box_loss + cls_loss + box_loss + mask_loss
    ...
    for epoch in range(epochs):
        ...
        for imgs, bboxes, num_b, num_H, num_W, masks in dataloader:
            ...
            if (step <= 10000 and step % 1000 == 0) or step % 5000 == 0 or step == 1:
                torch.save(model.state_dict(), './models/Mask_Rcnn_cascade_101_%d_1.pth' % step)
            if step >= 90010:
                flag = True
                break
        if flag:
            break
    torch.save(model.state_dict(), './models/Mask_Rcnn_cascade_101_final_1.pth')
...
def train_dist(model, config, step, x, pre_model_file, model_file=None):
        ...
        for imgs, bboxes, num_b, num_H, num_W, masks in dataloader:
            ...
            if ((step <= 10000 and step % 1000 == 0) or step % 5000 == 0 or step == 1) and local_rank == 0:
                torch.save(model.module.state_dict(),
                           './models/Mask_Rcnn_cascade_101_%dx_%d_1_%d.pth' % (x, step, local_rank))
            if step >= 90010 * x:
                flag = True
                break
        if flag:
    ...
    if local_rank == 0:
        torch.save(model.module.state_dict(),
                   './models/Mask_Rcnn_cascade_101_%dx_final_1_%d.pth' % (x, local_rank))
...
if __name__ == "__main__":
    ...
    model = Mask_Rcnn
    x = 4
    pre_model_file = r'D:\BaiduNetdiskDownload\pytorch_resnet_caffe/resnet101-caffe.pth'
